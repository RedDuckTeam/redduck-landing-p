      ...
      >
        ...
        {item.whatWeBuilt && (
          <div className="space-y-3">
            <h4 className="ibm-plex-mono text-base font-bold uppercase lg:text-[24px]">
              WHAT WE BUILT:
            </h4>
            <ul className="space-y-2 text-sm leading-[140%] md:text-base lg:text-[20px]">
              {item.whatWeBuilt.map((bullet) => (
                <li key={bullet} className="flex items-baseline gap-3">
                  <span
                    aria-hidden
                    className="block size-[6px] shrink-0 translate-y-[-1px] rounded-full bg-current"
                  />
                  <span>{bullet}</span>
                </li>
              ))}
